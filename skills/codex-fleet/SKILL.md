...
3. **For multiple independent jobs, fire them ALL in parallel — but say so first.** Codex sessions don't contend. Compute is not the bottleneck — throughput is. If the user asks for 4 images or 3 codex investigations, that's 4 or 3 simultaneous Bash calls in one message, all `run_in_background: true`. Before firing, state lane count and each lane's model/effort in one sentence — not a request for approval, just disclosure, since N heavy lanes can burn a Codex quota in minutes.
...
Model and effort are a per-lane decision made by the calling agent, not a
fixed default of this skill. When the caller hasn't stamped a lane spec:
...
which model/effort each gets — disclosure, not an approval gate, but it's the
only thing standing between "routine fleet" and "N heavy lanes just ate the
Codex quota."
